        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_WARLOCK:
            {
				                switch(m_spellInfo->Id)
                {
                    ...
                    case 3716:  // Torment
                    {
                        float spellpower = (float)(m_caster->GetOwner()->SpellBaseDamageBonus(SPELL_SCHOOL_MASK_SHADOW) + m_caster->GetOwner()->SpellBaseDamageBonusForVictim(SPELL_SCHOOL_MASK_SHADOW, unitTarget));
                    ...
                    case 3110: // Firebolt
                    {
                        if (m_caster->isPet())
                        {
                            float spellpower = (float)(m_caster->GetOwner()->SpellBaseDamageBonus(SPELL_SCHOOL_MASK_FIRE) + m_caster->SpellBaseDamageBonusForVictim(SPELL_SCHOOL_MASK_FIRE, unitTarget));
                            damage += int32(spellpower * 0.329f);

                            int32 bp0 = 0;
                            if (m_caster->GetOwner()->ToPlayer()->HasAura(91986)) // Burning Embers
                            {
                                bp0 = CalculatePctN(int32(damage), 15) /7;
                                m_caster->CastCustomSpell(unitTarget, 85421, &bp0, NULL, NULL, true);
                            }
                            if (m_caster->GetOwner()->ToPlayer()->HasAura(85112))
                            {
                                bp0 = CalculatePctN(int32(damage), 30) /7;
                                m_caster->CastCustomSpell(unitTarget, 85421, &bp0, NULL, NULL, true);
                            }
                            // Empowered Imp
	...
	case SPELLFAMILY_WARRIOR:
            ...
            switch (m_spellInfo->Id)
            {
                ...
                case 20252:
                {
					...
					{
						m_caster->CastSpell(m_caster, 100, false);
