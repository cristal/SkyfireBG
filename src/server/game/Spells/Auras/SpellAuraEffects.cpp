    ...
    switch (GetSpellInfo()->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DRUID:
        {
            switch (GetSpellInfo()->Id)
            {
                ...
                case 22842:
                {
                    // Converts up to 10 rage per second into health for $d.  Each point of rage is converted into ${$m2/10}.1.5% of max health.
                    // Should be manauser
                    ...
                    int32 regen = (target->GetMaxHealth() * (mod * points / 10) / 1000) / 200;
