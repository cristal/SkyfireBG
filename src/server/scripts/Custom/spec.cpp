...
public:
	...
	void GossipHelloSpec(Player* player, Creature* creature, uint32 arenaType)
	{
	    ...
	    {	
			for (BattlegroundSet::iterator itr = sBattlegroundMgr->GetAllBattleGrounds(i).begin(); itr != sBattlegroundMgr->GetAllBattleGrounds(i).end(); itr++)
          {
			sLog->outString("4.");
	...
	void SpectatePlayer(Player* player, Player* target, Battleground* bg)
	{
		...
		player->SetGameMaster(PLAYER_EXTRA_GM_ON);
...
public:
	...
	static bool HandleLeaveCommand(ChatHandler * handler, const char * args)
    {
		...
		if(player->IsVisible() == false &&  player->GetSpeed(MOVE_RUN) == 4)
		{
